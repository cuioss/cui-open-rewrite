...
* **Detects incorrect logger naming** - Flags logger fields not named `LOGGER` (uppercase)
* **Detects incorrect logger modifiers** - Flags loggers not declared as `private static final`
* **Detects incorrect placeholder patterns** - Flags `{}` (SLF4J) and `%d`, `%f`, etc. instead of `%s`
* **Detects wrong exception positioning** - Flags exception parameters not in first position for error/warn calls
* **Detects System.out/err usage** - Flags inappropriate console output
* **Validates parameter counts** - Warns when placeholder count doesn't match parameter count
...
* **Validates LogRecord usage by level** - Ensures LogRecord is used only where appropriate
* **Detects missing LogRecord** - Flags INFO/WARN/ERROR/FATAL calls without LogRecord
* **Detects inappropriate LogRecord** - Flags DEBUG/TRACE calls using LogRecord
