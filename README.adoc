...
public class YourRecipe extends Recipe {
    ...
    private static class YourRecipeVisitor extends BaseSuppressionVisitor {
        ...
        // BaseSuppressionVisitor handles class/method level suppression automatically
        @Override
        public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
            J.MethodInvocation mi = super.visitMethodInvocation(method, ctx);
            
            // Check for element-level suppression (comments directly on this method invocation)
...
* Eliminates code duplication for class and method level suppression patterns
* Automatically handles class-level and method-level suppression (no need to check in visitClassDeclaration/visitMethodDeclaration)
* Consistent suppression behavior across all recipes
* Reduces boilerplate code (but element-level suppression checks still needed in specific visit methods)
