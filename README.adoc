...
* **Auto-fixes logger naming** - Renames logger fields to `LOGGER` (uppercase)
* **Auto-fixes logger modifiers** - Changes visibility to `private static final`
* **Auto-fixes placeholder patterns** - Replaces `{}` (SLF4J) and `%d`, `%f`, etc. with `%s`
* **Auto-fixes exception positioning** - Moves exception parameters to first position in error/warn calls
* **Detects System.out/err usage** - Flags inappropriate console output with warnings
...
public class Example {
    public static CuiLogger log = new CuiLogger(Example.class);
    
    void method(Exception e) {
        System.out.println("Debug message");
        log.error("Error {} occurred", "Database", e);
        log.info("Count: %d", 42);
    }
}
...
public class Example {
    ...
    void method(Exception e) {
        System.out.println("Debug message"); // Warning: Use proper logging
        LOGGER.error(e, "Error %s occurred", "Database");
        LOGGER.info("Count: %s", 42);
    }
}
----
